...
class _rasterize_func(torch.autograd.Function):
    @staticmethod
    def forward(ctx, raster_ctx, pos, tri, resolution, ranges, grad_db, backface_culling, peeling_idx):
        if isinstance(raster_ctx, RasterizeGLContext):
            out, out_db = _get_plugin(gl=True).rasterize_fwd_gl(raster_ctx.cpp_wrapper, pos, tri, resolution, ranges, backface_culling, peeling_idx)
        else:
            out, out_db = _get_plugin().rasterize_fwd_cuda(raster_ctx.cpp_wrapper, pos, tri, resolution, ranges, backface_culling, peeling_idx)
    ...
    def backward(ctx, dy, ddb):
        ...
        return None, g_pos, None, None, None, None, None, None

# Op wrapper.
def rasterize(glctx, pos, tri, resolution, ranges=None, grad_db=True, backface_culling = 0):
    '''Rasterize triangles.
    ...
    Args:
        ...
        backface_culling: optional, 0 is none, 1 is closewise, 2 is counter-clockwise
    ...
    return _rasterize_func.apply(glctx, pos, tri, resolution, ranges, grad_db, backface_culling, -1)
