...
std::tuple<torch::Tensor, torch::Tensor> rasterize_fwd_cuda(RasterizeCRStateWrapper& stateWrapper, torch::Tensor pos, torch::Tensor tri, std::tuple<int, int> resolution, torch::Tensor ranges, int backface_culling, int peeling_idx)
{
    ...
    NVDR_CHECK(backface_culling >= 0 && backface_culling <= 2, "backface_culling is >=0, <=2");
    ...
    p.height = height;
    p.depth  = depth;
    p.backface_culling = backface_culling;
