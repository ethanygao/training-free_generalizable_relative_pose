...
std::tuple<torch::Tensor, torch::Tensor> rasterize_fwd_gl(RasterizeGLStateWrapper& stateWrapper, torch::Tensor pos, torch::Tensor tri, std::tuple<int, int> resolution, torch::Tensor ranges, int backface_culling, int peeling_idx)
{
    ...
    NVDR_CHECK(backface_culling >= 0 && backface_culling <= 2, "backface_culling must between [0, 2]");
    ...
    // Resize all buffers.
    bool changes = false;
    rasterizeResizeBuffers(NVDR_CTX_PARAMS, s, changes, posCount, triCount, width, height, depth, backface_culling);
    if (changes)
    {
#ifdef _WIN32
