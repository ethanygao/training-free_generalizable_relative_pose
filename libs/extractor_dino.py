...
class ViTExtractor:
    ...
    def __init__(self, model_type: str = 'dino_vits8', stride: int = 4, model: nn.Module = None, device: str = 'cuda'):
        ...
        if 's' in self.model_type:
            self.attn_layer = 11
        elif 'l' in self.model_type:
            self.attn_layer = 23
        ...
        self.transform1 = transforms.Compose([transforms.Resize((518,518)), 
                                              transforms.ToTensor(), 
                                              transforms.Normalize(mean=0.5, std=0.2) ])
                                     
        self.transform = transforms.Compose([           
                                transforms.Resize((518,518)), # or 840??                 
                                transforms.ToTensor(),                    
    ...
    def create_model(model_type: str) -> nn.Module:
        ...
        if 'v2' in model_type:
            model = torch.hub.load(f'{ROOT_dir}/third_party/dinov2', model_type, source='local', pretrained=False)
            model.load_state_dict(torch.load(f'{ROOT_dir}/third_party/dinov2/weights/{model_type}_pretrain.pth'))
    ...
    def extract_attn(self, batch: torch.Tensor, layer: int = 11, facet: str = 'attn',
                            bin: bool = False, include_cls: bool = False) -> torch.Tensor:
        """
        extract descriptors from the model
        :param batch: batch to extract descriptors for. Has shape BxCxHxW.
        ...
        if not include_cls:
            desc = x[:, :, 1:, 1:]  # remove cls token
        else:
            desc = x
        return desc

    def extract_saliency_maps_(self, batch: torch.Tensor) -> torch.Tensor:
        """
    ...
    def get_correspondences(self, ref_feats, target_feats, ref_attn, target_attn, device, num_correspondences = 50, correspondence_mode = 'batch_knn', high_res = False):
        """
        Args:
        ...
        if correspondence_mode == 'batch':
            selected_points_image_2, selected_points_image_1, cyclical_dists, sim_selected_12 = self.find_correspondences_batch(
                descriptors1=target_feats,
                descriptors2=ref_feats,
                attn1=target_attn,
                attn2=ref_attn,
                device=device,
                num_pairs=num_correspondences)

        elif correspondence_mode == 'batch_knn':
            if high_res:
                num_pairs_for_topk = 8 * num_correspondences
            else:
                num_pairs_for_topk = 2 * num_correspondences

            selected_points_image_2, selected_points_image_1, cyclical_dists, sim_selected_12 = \
                self.find_correspondences_batch_with_knn(
                descriptors1=target_feats,
                descriptors2=ref_feats,
                attn1=target_attn,
                attn2=ref_attn,
...
def build_dino_extractor(dino_model_size = 'large'):
    model_dict={'small':'dinov2_vits14',
                'base':'dinov2_vitb14',
                'large':'dinov2_vitl14',
                'giant':'dinov2_vitg14'}
    
    model_type = model_dict[dino_model_size]
...
def get_pca_image_dinov2(extractor, img_pil, img_mask, 
    ...
    transform1 = extractor.transform1
    attn_layer = extractor.attn_layer
